...
public class PsugoSendClientDataHelper extends AsyncTask<Context, String, String> {
 ...
 //   	final String NAME_SPACE = "http://wally.v3w.net";
	...
	private void debugInstitution(String fromMethod, Institution myInstitution) {
		...
		int nPhotos = myInstitution.photo.length;
	...
	public String envoyerInstitution() throws Exception {
		...
		for (int i = 0; i < instSize; i++) {
			if ( myDbInst[i].photo != null  && myDbInst[i].photo.length > 0) {
				System.out.println("Debug EnvoyerInstitution" + myDbInst[i].photo.length);
			}
			// ne pas transmettre si pas de photos... ou bien si l'utilisateur n'a pas mis a jour
			if ( myDbInst[i].photo != null  && myDbInst[i].photo.length > 0 && myDbInst[i].photo[0] != null){
				//*********PATCH********* TO FIX
	...
	protected String doInBackground(Context... params) {
		...
		try {
			...
			if (retCode == 1) {
				retCode = Integer.parseInt(this.envoyerInstitution());
